...
// 클래스는 구조체보다 느리다. 동적 디스패치때문에. 뷰컨트롤러를 만들 때 일반적으로 final 키워드 사용
final class ViewController: UIViewController {
    
    //private : 외부 접근 불가하도록, 객체지향 캡슐화 관점에서
    // MARK: - 이메일 입력하는 텍스트 뷰
    ...
    // lazy var 사용해야만 addSubview 사용가능. 올리려는 서브뷰보다 먼저 실행되면 안되므로.
    ...
    // 오토레이아웃을 변수에 담아놓은 후에 동적으로 변경해 사용.
    ...
    private func configure() {
        ...
        // 텍스트 필드를 사용하기 위해 델리게이트(대리자) 설정
        // 이후 UItextField 프로토콜을 채택해 구현된 사항을 사용 가능
